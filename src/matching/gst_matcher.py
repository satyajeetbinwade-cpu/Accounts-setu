...
def _normalize_invoice_number(raw: str) -> str:
    """Uppercase, strip spaces and common separators, strip leading zeros
    from numeric segments. E.g. 'INV/0023/24-25' → 'INV232425'."""
...
def _trailing_numeric_key(raw: str) -> str | None:
    """Extract the last numeric run from the invoice number. Returns None
    if no numeric segment exists."""
    nums = re.findall(r"\d+", str(raw).strip())
    if not nums:
        return None
...
def preprocess_portal(
    ...
    df["_norm_inv"] = df["invoice_number"].apply(_normalize_invoice_number)
    df["_trail_num"] = df["invoice_number"].apply(_trailing_numeric_key)
...
def preprocess_books(books_df: pd.DataFrame) -> pd.DataFrame:
...
def _compare_pair(
    ...
    if rate_b is not None and rate_p is not None:
        if not _rate_is_valid(rate_b, config) or not _rate_is_valid(rate_p, config):
            detail = (
                f"Rate Mismatch: books implies {rate_b}%, portal implies {rate_p}%; "
                f"one or both do not correspond to a standard GST slab."
            )
...
def _resolve_ambiguity(
    ...
    best_score, best_row, best_frag = candidates[0]
    forced_low = False
    note = ""
    ...
    if len(candidates) > 1:
        ...
        if gap < margin:
            ...
            note = (
                ...
                f"chosen on highest score ({best_score:.0f} vs runner-up {runner_score:.0f}); "
    ...
    best_score = max(0.0, min(100.0, round(best_score, 2)))
    return best_row, best_score, note, forced_low
    ...
    for _, br in _unmatched_books().iterrows():
        ...
        if candidates:
            ...
            r = _make_pair_result(
                ...
                extra_reason=amb_note, forced_low=forced,
            )
            results.append(r)
...
def match_gst(
    ...
    *,
    ...
    """
    # --- Pre-processing ---
    books = preprocess_books(books_df)
